            ...
            <div class="flex justify-between items-center">
                ...
                <div>
                    <?php if ($order->status === 'pending'): ?>
                        <p class="status-badge rounded-xl bg-[#ffbe00] text-white text-[10px] px-[15px] py-[5px]">
                            Pending
                        </p>
                    <?php elseif ($order->status === 'confirmed'): ?>
                        <p class="status-badge flex justify-center rounded-2xl bg-teal-500 text-white text-[10px] px-[15px] py-[5px]">
                            Confirmed
                        </p>
                    <?php elseif ($order->status === 'assembled'): ?>
                        <p class="status-badge flex justify-center rounded-2xl bg-[#F28C28] text-white text-[10px] px-[15px] py-[5px]">
                            Assembled
                        </p>
                    <?php elseif ($order->status === 'shipped'): ?>
                        <p class="status-badge flex justify-center rounded-2xl bg-indigo-500 text-white text-[10px] px-[15px] py-[5px]">
                            Shipped
                        </p>
                    <?php elseif ($order->status === 'paid'): ?>
                        <p class="status-badge flex justify-center rounded-2xl bg-lime-500 text-white text-[10px] px-[15px] py-[5px]">
                            Paid
                        </p>
                    <?php elseif ($order->status === 'cancelled'): ?>
                        <p class="status-badge flex justify-center rounded-2xl bg-red-500 text-white text-[10px] px-[15px] py-[5px]">
                            Cancelled
                        </p>
                    <?php elseif ($order->status === 'delivered'): ?>
                        <p class="status-badge flex justify-center rounded-2xl bg-green-700 text-white text-[10px] px-[15px] py-[5px]">
                            Delivered
                        </p>
                    <?php endif ?>
                </div>
            ...
            <div class="my-3">
                ...
                <div class="flex flex-wrap gap-1 text-xs text-gray-600">
                    <?php foreach ($order->orderItems as $orderItem): ?>
                        <span class="bg-gray-100 px-2 py-1 rounded text-sm mt-1">
            ...
            <div class="flex flex-wrap gap-2 items-center mb-2">
                <?php if ($order->status === 'pending'): ?>
                    <form method="POST" action="/admin/orders/<?= $order->id ?>/update-status" class="inline">
                        ...
                        <button type="submit" onclick="updateProgress(<?= $order->id ?>, 'confirmed')" class="bg-yellow-500 text-white px-4 py-1 rounded text-sm hover:bg-yellow-600">
                            Confirm Order
                        </button>
                    </form>
                    <form method="POST" action="/admin/orders/<?= $order->id ?>/cancel" class="inline">
                        <?= csrf_token() ?>
                        <button type="submit" onclick="updateProgress(<?= $order->id ?>, 'cancelled')" class="bg-red-500 hover:bg-red-700 text-white px-4 py-1 rounded text-sm">
                            Cancel
                        </button>
                    </form>
                <?php elseif ($order->status === 'confirmed'): ?>
                    <form method="POST" action="/admin/orders/<?= $order->id ?>/update-status" class="inline">
                        ...
                        <button type="submit" onclick="updateProgress(<?= $order->id ?>, 'assembled')" class="bg-[#F28C28] hover:bg-[#a05f1e] text-white px-4 py-1 rounded text-sm">
                            Mark Assembled
                        </button>
                    </form>
                <?php elseif ($order->status === 'assembled'): ?>
                    <form method="POST" action="/admin/orders/<?= $order->id ?>/update-status" class="inline">
                        ...
                        <button type="submit" onclick="updateProgress(<?= $order->id ?>, 'shipped')" class="bg-indigo-500 hover:bg-indigo-700 text-white px-4 py-1 rounded text-sm">
                            Mark Shipped
                        </button>
                    </form>
                <?php elseif ($order->status === 'shipped'): ?>
                    <form method="POST" action="/admin/orders/<?= $order->id ?>/update-status" class="inline">
                        ...
                        <button type="submit" onclick="updateProgress(<?= $order->id ?>, 'delivered')" class="bg-green-700 hover:bg-green-800 text-white px-4 py-1 rounded text-sm">
                            Mark Delivered
                        </button>
                    </form>
                <?php elseif ($order->status === 'delivered'): ?>
                    <form method="POST" action="/admin/orders/<?= $order->id ?>/update-status" class="inline">
                        ...
                        <button type="submit" onclick="updateProgress(<?= $order->id ?>, 'paid')" class="bg-yellow-500 text-white px-4 py-1 rounded text-sm">
                            Mark Paid
                        </button>
                    </form>
                <?php endif ?>
            ...
            <div id="progress-timeline-<?= $order->id ?>" class="mt-4 hidden border-t border-gray-200">
                ...
                <div class="relative space-y-3">
                    ...
                    <div class="flex items-center gap-3 relative" data-step="paid">
                        ...
                        <span class="step-text font-medium text-gray-600">
