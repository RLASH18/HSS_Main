...
<div class="mb-4 sm:mb-8">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold text-black-900 mb-2">My Orders</h1>
            <p class="text-sm sm:text-base text-black-600">View your order history and status updates</p>
        </div>
...
<?php if (empty($orders)): ?>
    <!-- Empty cart -->
    <div class="p-6 sm:p-12 text-center bg-white border border-gray-100 rounded-lg shadow-sm">
        ...
        <h3 class="mb-2 text-xl font-semibold text-gray-900">No orders yet</h3>
        <p class="mb-6 text-gray-600">Looks like you haven’t placed any orders yet. Start shopping to place your first order.</p>
...
<?php else: ?>
    <div id="order-list">
        <div class="list space-y-4 sm:space-y-6">
            <?php foreach ($orders as $order) : ?>
                <div class="border rounded-xl shadow-sm p-3 sm:p-4 lg:p-6 bg-white">
                    <!-- Order header -->
                    <div class="flex justify-between items-start mb-4 sm:mb-6">
                        <div class="flex-1">
                            <h2 class="order-name font-semibold text-base sm:text-lg text-gray-800 mb-2">Order #<?= str_pad($order->id, 4, '0', STR_PAD_LEFT) ?></h2>
                            <div class="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-6 text-xs sm:text-sm text-gray-500">
                                ...
                                <div class="flex items-center">
                                    ...
                                    Delivery by <?= date('M d, Y', strtotime($order->created_at . ' +3 days')) ?>
                                </div>
                            </div>
                        </div>
                        <div>
                    ...
                    <?php if ($order->status === 'cancelled'): ?>
                        <!-- Cancelled Order Design -->
                        <div class="mb-4 sm:mb-6 p-3 sm:p-4 bg-red-50 border border-red-200 rounded-lg">
                            <div class="flex items-center justify-center text-center">
                                <div class="flex flex-col items-center">
                                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-full bg-red-500 flex items-center justify-center mb-2 sm:mb-3">
                                    ...
                                    <h3 class="text-base sm:text-lg font-semibold text-red-700 mb-1">Order Cancelled</h3>
                                    <p class="text-xs sm:text-sm text-red-600 mb-2">This order has been cancelled by the administrator</p>
                                    <div class="text-xs text-red-500">
                                        Cancelled on <?= date('M d, Y \a\t g:i A', strtotime($order->updated_at)) ?>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <?php else: ?>
                        <div class="progress-container mb-4 sm:mb-6 overflow-x-auto" data-status="<?= strtolower($order->status) ?>">
                            <div class="flex items-start justify-between relative min-w-max sm:min-w-0 px-3 sm:px-0">
                                <!-- Progress Line -->
                                <div class="absolute top-3 sm:top-4 left-0 right-0 h-0.5 bg-gray-200 z-0 mx-[3rem] sm:mx-[10%]">
                                    <div class="progress-line h-full bg-[#815331] transition-all duration-500" style="width: 0%"></div>
                                </div>

                                <!-- Status steps -->
                                <div class="step flex flex-col items-center justify-start relative z-10 flex-1" data-step="pending">
                                    <div class="step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500">
                                    ...
                                    <span class="step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500 whitespace-nowrap">Order Placed</span>
                                </div>

                                <div class="step flex flex-col items-center justify-start relative z-10 flex-1" data-step="confirmed">
                                    <div class="step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500">
                                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-white" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"></path>
                                        </svg>
                                    </div>
                                    <span class="step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500 whitespace-nowrap">Confirmed</span>
                                </div>

                                <div class="step flex flex-col items-center justify-start relative z-10 flex-1" data-step="assembled">
                                    <div class="step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500">
                                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                    ...
                                    <span class="step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500 whitespace-nowrap">Assembled</span>
                                </div>

                                <div class="step flex flex-col items-center justify-start relative z-10 flex-1" data-step="shipped">
                                    <div class="step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500">
                                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                            <path d="M8.96456 18C8.72194 19.6961 7.26324 21 5.5 21C3.73676 21 2.27806 19.6961 2.03544 18H1V6C1 5.44772 1.44772 5 2 5H16C16.5523 5 17 5.44772 17 6V8H20L23 12.0557V18H20.9646C20.7219 19.6961 19.2632 21 17.5 21C15.7368 21 14.2781 19.6961 14.0354 18H8.96456ZM15 7H3V15.0505C3.63526 14.4022 4.52066 14 5.5 14C6.8962 14 8.10145 14.8175 8.66318 16H14.3368C14.5045 15.647 14.7296 15.3264 15 15.0505V7ZM17 13H21V12.715L18.9917 10H17V13ZM17.5 19C18.1531 19 18.7087 18.5826 18.9146 18C18.9699 17.8436 19 17.6753 19 17.5C19 16.6716 18.3284 16 17.5 16C16.6716 16 16 16.6716 16 17.5C16 17.6753 16.0301 17.8436 16.0854 18C16.2913 18.5826 16.8469 19 17.5 19ZM7 17.5C7 16.6716 6.32843 16 5.5 16C4.67157 16 4 16.6716 4 17.5C4 17.6753 4.03008 17.8436 4.08535 18C4.29127 18.5826 4.84689 19 5.5 19C6.15311 19 6.70873 18.5826 6.91465 18C6.96992 17.8436 7 17.6753 7 17.5Z" />
                                        </svg>
                                    </div>
                                    <span class="step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500 whitespace-nowrap">Shipped</span>
                                </div>

                                <div class="step flex flex-col items-center justify-start relative z-10 flex-1" data-step="delivered">
                                    <div class="step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500">
                                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                            <path d="M12 20.8995L16.9497 15.9497C19.6834 13.2161 19.6834 8.78392 16.9497 6.05025C14.2161 3.31658 9.78392 3.31658 7.05025 6.05025C4.31658 8.78392 4.31658 13.2161 7.05025 15.9497L12 20.8995ZM12 23.7279L5.63604 17.364C2.12132 13.8492 2.12132 8.15076 5.63604 4.63604C9.15076 1.12132 14.8492 1.12132 18.364 4.63604C21.8787 8.15076 21.8787 13.8492 18.364 17.364L12 23.7279ZM12 13C13.1046 13 14 12.1046 14 11C14 9.89543 13.1046 9 12 9C10.8954 9 10 9.89543 10 11C10 12.1046 10.8954 13 12 13ZM12 15C9.79086 15 8 13.2091 8 11C8 8.79086 9.79086 7 12 7C14.2091 7 16 8.79086 16 11C16 13.2091 14.2091 15 12 15Z" />
                                        </svg>
                                    </div>
                                    <span class="step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500 whitespace-nowrap">Delivered</span>
                                </div>

                                <div class="step flex flex-col items-center justify-start relative z-10 flex-1" data-step="paid">
                                    <div class="step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500">
                                        <svg class="w-3 h-3 sm:w-4 sm:h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor">
                                    ...
                                    <span class="step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500 whitespace-nowrap">Payment Complete</span>
                                </div>
                            </div>
                        </div>
                    <?php endif ?>
                    ...
                    <div class="mb-3 sm:mb-4">
                        <h3 class="text-sm sm:text-base font-medium text-gray-800 mb-2 sm:mb-3">Order Items</h3>
                        <div class="space-y-2 sm:space-y-3">
                            <?php foreach ($order->orderItems as $orderItem): ?>
                                <div class="flex justify-between items-center py-2 px-2 sm:px-3 bg-gray-100 rounded-lg">
                                    <div class="flex-1 min-w-0">
                                        <span class="text-sm sm:text-base text-gray-800 font-medium line-clamp-2"><?= $orderItem->items->item_name ?></span>
                                        <div class="text-xs sm:text-sm text-gray-500">Qty: <?= $orderItem->quantity ?></div>
                                    </div>
                                    <div class="text-right ml-2">
                                        <span class="text-sm sm:text-base font-medium text-gray-900">₱<?= number_format($orderItem->unit_price * $orderItem->quantity, 2) ?></span>
                                    </div>
                                </div>
                            <?php endforeach ?>
                        </div>
                    ...
                    <!-- Order total -->
                    <div class="flex justify-between items-center pt-2 sm:pt-3 border-t border-gray-200">
                        <span class="text-sm sm:text-base font-semibold text-gray-800">Total</span>
                        <span class="font-bold text-lg sm:text-xl text-[#815331]">₱<?= number_format($order->total_amount, 2) ?></span>
                    </div>
                </div>
            <?php endforeach ?>
        </div>
        ...
        document.querySelectorAll('.progress-container').forEach(container => {
            ...
            steps.forEach((step, index) => {
                ...
                if (index <= currentIndex) {
                    // Completed step
                    circle.className = 'step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-[#815331] text-white';
                    label.className = 'step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-[#815331] font-medium';
                } else {
                    // Pending step
                    circle.className = 'step-circle w-6 h-6 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-sm font-medium bg-gray-300 text-gray-500';
                    label.className = 'step-label text-[10px] sm:text-xs mt-1 sm:mt-2 text-center text-gray-500';
                }
            });
        });
    });
...
<script>
    // Custom Pagination with List.js
    document.addEventListener('DOMContentLoaded', function() {
        // Initialize List.js without default pagination
        var options = {
            valueNames: ['order-name', 'status'],
            page: 5,
